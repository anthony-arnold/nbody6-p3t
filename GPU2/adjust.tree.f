...
*     Add any contributions from chain regularization.
      WRITE(6, *) 'NCH ECH', NCH, ECH
      IF (NCH.GT.0) THEN
          ETOT = ETOT + ECH
      END IF
      WRITE(6,*) 'ENERGY ', TIME, ETOT, ZKIN, POT, ETIDE, EPL, EBIN,
     &     ESUB, EMERGE, ECOLL, EMDOT, ECDOT
      FLUSH(6)
*
...
*       Determine average neighbour number and smallest neighbour sphere.
   ...
   30 CONTINUE
      ZNNB = FLOAT(NNB)/(N - NPAIRS)
      NNB = NNB/(N - NPAIRS)
*
*
*     Determine ideal neighbour radius
*
*      RS0 = (1.0D0/64.0D0) * (NTOT / 2.0D0**16)**(-1.0D0/3.0D0)
*       Use current value if minimum neighbour sphere not implemented.
c     IF (RSMIN.EQ.0.0D0) RSMIN = RS0
      RSMIN = RS(IFIRST)
*
...
c$$$          DO 44 I = IFIRST,NTOT
c$$$              IF (STEPR(I).GT.SMAX) THEN
c$$$                  STEPR(I) = SMAX
c$$$                  STEP(I) = MIN(STEP(I),SMAX)
c$$$                  TNEW(I) = STEP(I)
c$$$              END IF
c$$$   44     CONTINUE
      END IF
...
*
      IF (TTOT.EQ.0.0) THEN
         ...
         TINTRE = 0.0
      ELSE
         WRITE(6,46) TINTIR, TINTRE
 46      FORMAT(/,' Integration IRR Time =', E11.3,
     &            ' REG Time =', E11.3)
         CALL FLUSH(6)
      END IF
...
*     Output current state
      IF (KZ(50).GT.1) THEN
         ...
         DO I = IFIRST,NTOT
            WRITE(98,*) NAME(I),BODY(I),
     &                  (X(K,I),K=1,3),(XDOT(K,I),K=1,3),
     &                  (FI(K,I),K=1,3),(FIDOT(K,I),K=1,3),
     &                  (FR(K,I),K=1,3),(FRDOT(K,I),K=1,3)
         END DO
         CLOSE(98)
      END IF
*
...
*       Include optional fine-tuning of neighbour number (#40 >= 2).
      ...
      END IF
*
*     Estimate the ideal neighbour radius.
c      IF (ABS((NNBOPT-ZNNB)/ZNNB).GT.0.1) THEN
c         RSNEXT = RS0 * (FLOAT(NNBOPT) / ZNNB)**ONE3
c         WRITE(6,*) 'RSNEXT = ', RSNEXT
c      END IF
*

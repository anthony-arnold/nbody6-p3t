...
*
     ...
     &               NAMEC(NTMAX)
      ...
      REAL*8  EI0(3),EI(3),XX(3,3),VV(3,3),UJDOT(4),UKDOT(4)
      LOGICAL IQ,ICHAIN
      SAVE ITERM
      SAVE  EI0,IT
...
*       Check optional BH condition (prevents mass-loss complications).
          ...
          END IF
*
*       Search the perturbers (singles and binaries) inside 5*SEMI.
  ...
  81      JCLOSE = 0
     ...
     &             (X(3,I)-X(3,J))*(XDOT(3,I)-XDOT(3,J))
              IF (RD.GE.0.0.AND.GI.LT.0.05) GO TO 85
     ...
     &                   (XDOT(3,I) - XDOT(3,J))**2
              ...
              END IF
   85     CONTINUE
*       Increase search distance for JCMAX = 0 (which also works).
          IF (JCMAX.EQ.0.AND.ITT.EQ.0) THEN
              ITT = ITT + 1
              RCR2 = 4.0*RCR2
*             WRITE (6,83)  JCLOSE, LIST(1,I1), SEMI, SQRT(RCR2), GI
*  83         FORMAT (' ENLARGE CHAIN    JCL NP A RCR G ',2I6,1P,5E10.2)
              IF (RCR2.LT.1.0D-04) GO TO 81
          END IF
*
*       Exclude zero or hierarchical perturber and also large mass ratio.
          IF (JCLOSE.LT.IFIRST) GO TO 88
          IF (NAME(JCLOSE).LE.0) GO TO 88
          IF (JCMAX.GT.0.AND.JCMAX.LT.IFIRST) GO TO 88 ! Deny JCMAX < IFIRST.

...
template <typename T>
static __device__ __forceinline__ T shuffle(T var, int src) {
    return __shfl_sync(0xffffffff, var, src);
}
template <typename T>
static __device__ __forceinline__ T shuffle(T var, int src, int width) {
...
static __device__ __forceinline__ void computeDensityAndNgb(
    ...
    float &density, float &nb, int* nblist, int nbmax, int idx)
{
...
static __device__ __forceinline__ int2 warpIntExclusiveScan(const int value)
{
  ...
  return make_int2(sum-value, shuffle(sum, WARP_SIZE-1, WARP_SIZE));
}

static __device__ __forceinline__ int2 warpBinExclusiveScan(const bool p)
{
  const unsigned int b = __ballot_sync(0xFFFFFFFF, p);
    ...
    const int packed_value, const int carryValue)
{
  ...
  return make_int2(val + (carryValue & (-(laneId < dist_block))), shuffle(val, WARP_SIZE-1, WARP_SIZE));
}
    ...
    int idx)
{
  ...
  const float rinv   = r2eps == 0.0f ? 0.0f : rsqrtf(r2eps);
  ...
  computeDensityAndNgb(r2,pos.w,massj,density.x,density.y, nb, nbmax, idx);
#endif
...
static __device__ __forceinline__ void directAcc(
    ...
    int nbmax)
{
  const float4 M0 = (FULL || ptclIdx >= 0) ? tex1Dfetch(texBody, ptclIdx)
      : make_float4(HUGE, HUGE, HUGE, 0.0f);

//#pragma unroll
  for (int j = 0; j < WARP_SIZE; j++)
  {
    const float4 jM0 = make_float4(shuffle(M0.x, j), shuffle(M0.y, j), shuffle(M0.z, j), shuffle(M0.w,j));
    const int jIdx = shuffle(ptclIdx, j);
    const float  jmass = jM0.w;
    const float3 jpos  = make_float3(jM0.x, jM0.y, jM0.z);
#pragma unroll
    for (int k = 0; k < NI; k++)
        acc_i[k] = add_acc(acc_i[k], pos_i[k], jmass, jpos, eps2, density_i[k], nb_i[k], nbmax, jIdx);
  }
}
    ...
    float2 &density)
{
  ...
  const float rinv  = r2 == 0.0f ? 0.0f : rsqrtf(r2);
  const float rinv2 = rinv *rinv;
    ...
    const float eps2)
{
  ...
  {
    const float4 jM0 = make_float4(shuffle(M0.x, j), shuffle(M0.y, j), shuffle(M0.z, j), shuffle(M0.w,j));
    const float4 jQ0 = make_float4(shuffle(Q0.x, j), shuffle(Q0.y, j), shuffle(Q0.z, j), 0.0f);
    const float4 jQ1 = make_float4(shuffle(Q1.x, j), shuffle(Q1.y, j), shuffle(Q1.z, j), 0.0f);
    const float  jmass = jM0.w;
    const float3 jpos  = make_float3(jM0.x, jM0.y, jM0.z);
#pragma unroll
      for (int k = 0; k < NI; k++)
...
#pragma unroll 1
  ...
  while (nCells > 0)
  {
...
#if 1
    {
      ...
      while (nParticle > 0)
      {
        ...
        {
            directAcc<NI,true>(acc_i, pos_i, ptclIdx, eps2, dens_i, nb_i, nbmax);
          nParticle  -= WARP_SIZE;
          nProcessed += WARP_SIZE;
          if (INTCOUNT)
        ...
        else
        {
          ...
          {
            /* evalute cells stored in shmem */
              directAcc<NI,true>(acc_i, pos_i, tmpList[laneIdx], eps2, dens_i, nb_i, nbmax);
  ...
  if (directCounter > 0)
  {
      directAcc<NI,false>(acc_i, pos_i, laneIdx < directCounter ? directPtclIdx : -1, eps2, dens_i, nb_i, nbmax);
    if (INTCOUNT)
    ...
    int    nbmax)
{
  ...
  nblist_i[0] = &neighbours_out[body_i[0] * nbmax];
  if (ni > 1) {
      nblist_i[1] = &neighbours_out[body_i[1] * nbmax];
  }

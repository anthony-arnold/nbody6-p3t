	...
	for(int koff=0; koff<nnb; koff+=NBMAX){
		...
		for(int k=0; k<nk; k+=4){
			...
			v4sf rv = dx*dvx + dy*dvy + dz*dvz;
                        // Set r2 small if mass is zero.
                        // r2 &= (m > 0);
                        r2 = __builtin_ia32_andps(
                            r2,
                            __builtin_ia32_cmpps(v4sf(0.0f), mj, 0x1));

                        // r2 |= 1e-6 & (r2 + 1 == 1)
                        r2 = __builtin_ia32_orps(
                            r2,
                        ...
                        // Set rv to zero if mass is zero.
                        // rv = (rv * -3.0) & (m > 0);
                        rv = __builtin_ia32_andps(
                            rv * v4sf(-3.0f),
                            __builtin_ia32_cmpps(v4sf(0.0f), mj, 0x1));

...
static const char* aeps = getenv("EPS");
static const float feps = aeps ? static_cast<float>(atof(aeps)) : 0.0f;
static const float feps2 = feps*feps;
static const v4sf eps2 = (v4sf){ feps2, feps2, feps2, feps2 };
...
static v4sf cutoff(const v4sf rinv, const v4sf rcut) {
    static const v4sf zero = (v4sf){ 0.f, 0.f, 0.f, 0.f };
    static const v4sf one = (v4sf){ 1.f, 1.f, 1.f, 1.f };
    static const v4sf gamma = (v4sf){ 0.1f, 0.1f, 0.1f, 0.1f };
    const v4sf y  = __builtin_ia32_rcpps(rinv * rcut);
    v4sf x = (y - gamma) / (one - gamma);
    x = __builtin_ia32_maxps(x, zero);
    x = __builtin_ia32_minps(x, one);
    v4sf K;
    const v4sf x2 = x*x;
    const v4sf x3 = x2*x;
    const v4sf x4 = x2*x2;
    K = -((v4sf){20.0f, 20.0f, 20.0f, 20.0f}) * (x4*x3) +
        ((v4sf){70.0f, 70.0f, 70.0f, 70.0f}) * (x4*x2) -
        ((v4sf){84.0f, 84.0f, 84.0f, 84.0f}) * (x4*x) +
        ((v4sf){35.0f, 35.0f, 35.0f, 35.0f}) * x4;
...
struct Particle{
	...
	Particle(
		...
		const double _mass,
		const double _time)
		: vel(_vel), acc(_acc), jrk(_jrk), time(_time)
	{
	...
	void prefetch() const {
		...
		__builtin_prefetch(&jrk, rw , locality);
	}
...
struct Predictor{
			...
			const double ti)
	{
		...
		posH = p.posH;
		posL = v4sf(p.posL) + s0*(v4sf(p.vel) + s1 * (v4sf(p.acc) + s2 * v4sf(p.jrk)));
		vel = v4sf(p.vel) + s0*(v4sf(p.acc) + s1 * v4sf(p.jrk));
...
static void gpuirr_set_jp(
		...
		const double time)
{
	// ptcl[addr] = Particle(pos, vel, acc, jrk, mass, time);
	// Particle(pos, vel, acc, jrk, mass, time).sstore(&ptcl[addr]);
                ...
                const double rcut)
{
	// list[addr].prefetch<0,1>();
        const float rcutf = static_cast<float>(rcut);
        const v4sf rcut4 = (v4sf){ rcutf, rcutf, rcutf, rcutf };
	...
	for(int k=0; k<nnb; k+=4){
		...
		const v4sf r2 = dx*dx + dy*dy + dz*dz + eps2;
#if 0
		v4sf_print(jp.xH);
		v4sf_print(jp.yH);
		v4sf_print(jp.zH);
		...
		const v4sf mrinv3 = jp.mass * rinv * rinv2;
                const v4sf kmrinv3 = mrinv3 * cutoff(rinv, rcut4);

		ax.accumulate(kmrinv3 * dx);
		ay.accumulate(kmrinv3 * dy);
		az.accumulate(kmrinv3 * dz);
#if 0
		jx.accumulate(kmrinv3 * (dvx + alpha * dx));
		jy.accumulate(kmrinv3 * (dvy + alpha * dy));
		jz.accumulate(kmrinv3 * (dvz + alpha * dz));
#else
		jx.accumulate(kmrinv3 * dvx); jx.accumulate(alpha * (kmrinv3 * dx));
		jy.accumulate(kmrinv3 * dvy); jy.accumulate(alpha * (kmrinv3 * dy));
		jz.accumulate(kmrinv3 * dvz); jz.accumulate(alpha * (kmrinv3 * dz));
#endif
	}
...
#pragma omp parallel for reduction(+: nint) schedule(guided)
	for(int i=0; i<ni; i++){
            gpuirr_firr(addr[i]-1, accout[i], jrkout[i], rcut);
...
#pragma omp parallel
	{
		...
		for(int i=0; i<ni; i++){
			...
			if(tid == iaddr % nth){
                            gpuirr_firr(iaddr, accout[i], jrkout[i], rcut);
			}
			if(0 == tid){
...
extern "C"{
	...
	void gpuirr_set_jp_(
		...
		double *time)
	{
		gpuirr_set_jp((*addr)-1, pos, vel, acc, jrk, *mass, *time);
	}
	...
	{
		const double t0 = get_wtime();
		gpuirr_firr((*addr)-1, acc, jrk, *rcut);
		const double t1 = get_wtime();
		::time_onep += t1-t0;
	}
	void gpuirr_firr_vec_(
			...
			double jrk [][3],
                        double *rcut)
	{
            gpuirr_firr_vec(*ni, addr, acc, jrk, *rcut);
	}
